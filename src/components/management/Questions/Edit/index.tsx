...
const EditQuestion = ({ show }: IEditQuestionProps) => {
  ...
  useEffect(() => {
    ...
    if (questionSelected) {
      ...
      setDifficult(questionSelected.difficult);
      ...
      setAnswers(questionSelected.answers);
    }
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!questionSelected || !isValid || !discipline || !answers) {
      return;
    }

    const data: IQuestionDTO = {
      _id: questionSelected._id,
      discipline,
      statement,
      difficult,
      answers,
    ...
    const response = await updateQuestion(questionSelected._id, data);
